...
class CassandraCluster:
                    ...
                    for f in contents:
                        ...
                        if os.path.isfile(filepath):
                            if f.endswith('.cql'):
                                ...
                                CassandraCluster.updateReq(keyspace,reqid)
                            else:
    ...
    def baseline(path,keyspace):
        ...
        if not CassandraCluster.tableExists(keyspace, tablename):
    ...
    def doMigration(keyspace, reqid):
        ...
        if os.path.isdir(schemaroot):
            CassandraCluster.baseline(os.path.join(schemaroot, 'baseline'), keyspace, reqid)
            CassandraCluster.migrateschema(os.path.join(schemaroot, 'schema_migrations'), keyspace, reqid)
        else:
    ...
    def setupKeyspace(keyspace):
        ...
        if not CassandraCluster.tableExists(keyspace, 'schema_migrations'):
            try:
        ...
        if not CassandraCluster.tableExists(keyspace, 'schema_migration_requests'):
            try:
        ...
        CassandraCluster.requestmigration(keyspace)
