...
class Users(Resource):
    def post(self):
        try:
            ...
            return {'ServerError': 500, 'Message': 'There was an error fulfiling your request'}, 500
        return {}
...
class User(Resource):
    def get(self,username,org):
        try:
            ...
            results = session.execute(getUserQuery, (org,username)).current_rows
        except Exception as e:
            log.error('Exception on User/get: %s'%str(e))
            return {'ServerError': 500, 'Message': 'There was an error fulfiling your request'}, 500
        if len(results) == 0:
            return {'Message': 'No user matched "%s"@"%s"'%(username,org)}, 404
        elif len(results) == 1:
            ...
            return {'RequestError': 400, 'Message': 'Request returned too many results'}, 400
